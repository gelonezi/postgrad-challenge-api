...
public class DeleteClienteCommandHandlerTests
{
    ...
    public DeleteClienteCommandHandlerTests()
    {
        ...
        _handler = new DeleteClienteCommandHandler(_repositoryMock.Object, _uowMock.Object);
    }

    [Fact]
    public async Task Handle_Should_SoftDeleteCliente_When_ClienteExists()
    {
    ...
    [Fact]
    public async Task Handle_Should_ReturnBadRequest_When_ClienteIsAlreadyDeleted()
    {
        var clienteId = Guid.NewGuid();
        var cliente = Cliente.Criar("João Silva", null, "12345678909", "joao@example.com", "11987654321").Data!;
        cliente.Excluir();
